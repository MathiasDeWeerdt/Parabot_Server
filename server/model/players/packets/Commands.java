...
public class Commands implements PacketType {
	...
	public void playerCommands(Client c, String playerCommand) {
		...
		if (playerCommand.startsWith("spawn")) {
			for (int i = 0; i < c.playerLevel[3]; i++)
				c.sendMessage("This is not a spawn server.");
		}
		...
		if (playerCommand.startsWith("rimm")) {
		...
		if (playerCommand.equalsIgnoreCase("check")
			...
			try {
				...
				c.sendMessage("[GTL Vote] A SQL error has occured PLEASE REPORT TO ADMIN/OWNER.");
			}
		}
		...
		if (playerCommand.startsWith("withdraw")) {
			...
			if (amount == 2000000000 || amount > 2000000000) {
				c.sendMessage("You cannot withdraw more then 2B!");
				return;
			}
			if (c.inWild()) {
			...
			if (amount > 0 && c.getItems().freeSlots() == 0) {
				c.sendMessage("You don't have a free slot!");
				return;
			}
			if (amount < 1) {
				return;
		...
		if (playerCommand.startsWith("donate")) {
			c.getPA().sendFrame126("http://Parabot.net/donate", 12000);
		...
		if (playerCommand.startsWith("commands")) {
			...
			c.sendMessage("::qpsk, ::resetcommands, ::chill, ::rimm, ::download, ::save, ::help, ::help, ::display");
		}
		...
		if (playerCommand.startsWith("vote")) {
			c.getPA().sendFrame126("http://Parabot.net/vote", 12000);
			c.sendMessage("Donating helps the server in MANY WAYS!");
		}
		if (playerCommand.startsWith("highscores")) {
			c.getPA().sendFrame126("Parabot.net/highscores", 12000);
			c.sendMessage("Donating helps the server in MANY WAYS!");
		}
		if (playerCommand.startsWith("help")) {
			c.getPA().sendFrame126(
					"Parabot.net/forums/index.php?/forum/40-bug-reports/",
					12000);
			c.sendMessage("Donating helps the server in MANY WAYS!");
		}
		if (playerCommand.startsWith("download")) {
			c.getPA().sendFrame126("Parabot.net/Parabot.jar", 12000);
		}
		...
		if (playerCommand.startsWith("changepassword")
			...
			if (c.hasRecov >= 1) {
				try {
					String urlString = "http://Parabot.net/recovery/update.php?key=3293029420439231&username="
		...
		if (playerCommand.startsWith("rules")) {
			c.getPA().sendFrame126("Parabot.net/rules", 12000);
		...
		if (playerCommand.startsWith("yell")) {
			 ...
			 * c.sendMessage
			 * ("You must be a donator or have Yell-Rights to use this command!"
			 * ); c.sendMessage(
			 * "You can buy Yell-Rights for 10K Loyalty Points! Use ::purchaseyell"
			 * ); return; }
			 */
			if (c.yellTimer > 0 && c.playerRights == 4 && c.isDonator > 0) {
				c.sendMessage("You're unable to yell for another "
						+ c.yellTimer + " seconds.");
				return;
			}
			...
			for (int j = 0; j < PlayerHandler.players.length; j++) {
				if (PlayerHandler.players[j] != null) {
					...
					if (c.playerRights == 5 && !hasYelled) {
						c2.sendMessage("<col=FF0000><shad=0> [Helper]"
								+ c.playerName + ": "
								+ Misc.optimizeText(playerCommand.substring(5))
								+ "");
						hasYelled = true;
					} else if (c.playerRights == 1 && !hasYelled) {
						c2.sendMessage("<col=20B2AA><shad=0> [Moderator]"
								+ c.playerName + ": "
								+ Misc.optimizeText(playerCommand.substring(5))
								+ "");
						hasYelled = true;
					} else if (c.playerRights == 2 && !hasYelled) {
						c2.sendMessage("<col=FFFF64><shad=0> [Admin]"
								+ c.playerName + ": "
								+ Misc.optimizeText(playerCommand.substring(5))
								+ "");
						hasYelled = true;
							...
							&& !hasYelled) {
						c2.sendMessage("<shad=FF7F00> [Bronze Member]"
								+ c.playerName + ": "
								+ Misc.optimizeText(playerCommand.substring(5))
								+ "");
						hasYelled = true;
					} else if (c.isDonator == 2 && c.playerRights == 4
							&& !hasYelled) {
						c2.sendMessage("<col=E6E8FA><shad=0> [Silver Member]"
								+ c.playerName + ": "
								+ Misc.optimizeText(playerCommand.substring(5))
								+ "");
						hasYelled = true;
							...
							&& !hasYelled) {
						c2.sendMessage("<col=D9D919><shad=0> [Gold Member]"
								+ c.playerName + ": "
								+ Misc.optimizeText(playerCommand.substring(5))
								+ "");
						hasYelled = true;
					} else if (c.isDonator == 4 && !hasYelled
							|| c.isDonator == 5 && !hasYelled
							|| c.isDonator == 6 && c.playerRights == 4
							&& !hasYelled) {
						c2.sendMessage("<shad=0EBFE9> [Diamond Member]"
								+ c.playerName + ": "
								+ Misc.optimizeText(playerCommand.substring(5))
								+ "");
						hasYelled = true;
						...
						c2.sendMessage("<col=00FFFF><shad=0> [Owner]"
								...
								+ "");
						hasYelled = true;
					} else if (c.hasYell && c.playerRights == 0
							&& c.isDonator == 0 && !hasYelled) {
						c2.sendMessage("<shad=6081134>[Player]" + c.playerName
								+ ": "
								+ Misc.optimizeText(playerCommand.substring(5))
								+ "");
						hasYelled = true;
	...
	public void moderatorCommands(Client c, String playerCommand) {
		...
		if (playerCommand.startsWith("szone") && c.playerRights > 0) {
			c.getPA().startTeleport(2956, 3506, 0, "modern");
		}
	...
	public void ownerCommands(Client c, String playerCommand) {
		...
		if (playerCommand.startsWith("Parabot")) {
			for (int j = 0; j < PlayerHandler.players.length; j++) {
				if (PlayerHandler.players[j] != null) {
					Client p = (Client) PlayerHandler.players[j];
					p.forcedChat("#1 RSPS Parabot JOIN NOW AT Parabot.net!");
		...
		if (playerCommand.startsWith("allv")) {
			for (int j = 0; j < PlayerHandler.players.length; j++) {
				if (PlayerHandler.players[j] != null) {
					Client p = (Client) PlayerHandler.players[j];
					p.forcedChat("#1 RSPS Parabot JOIN NOW AT Parabot.org!");
					p.gfx0(1600);
